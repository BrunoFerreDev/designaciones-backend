...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const filterFieldsWrapper = document.getElementById("filter-fields-wrapper");
  ...
  // Modal 2: Manage referees available filtering & assignment
  ...
  btnApplyBulkFee.addEventListener("click", async () => {
    ...
    try {
      ...
      addToast("Montos del arancel actualizados.");
  ...
  // Refresh
  btnSearchRefresh.addEventListener("click", () => {
    executeSearch(true);
  });

  // Resumen Árbitros
  btnSearchSummary.addEventListener("click", openRefereesByDay);
  ...
  function initDatesAndYears() {
    ...
    // Monthly setup
    searchMonth.value = String(now.getMonth() + 1);

    searchYear.innerHTML = "";
    for (let y = yyyy - 2; y <= yyyy + 3; y++) {
  ...
  async function fetchInitOptions() {
    try {
      ...
      // Populate Referee Selector
      searchRefereeSelect.innerHTML = '<option value="" disabled selected>Seleccione un árbitro...</option>';
      ...
      sortedRefs.forEach(a => {
        ...
        searchRefereeSelect.appendChild(opt);
      });
      ...
      // Populate Cancha Selector
      searchCourtSelect.innerHTML = '<option value="" disabled selected>Seleccione una cancha...</option>';
      const sortedCourts = [...allCanchas].sort((a,b) => a.nombre.localeCompare(b.nombre));
      sortedCourts.forEach(c => {
        const opt = document.createElement("option");
        opt.value = c.id;
        opt.textContent = `${c.nombre} (${c.ciudad || "Sin ubicación"})`;
        searchCourtSelect.appendChild(opt);
      });
    } catch (err) {
      console.error(err);
    }
  ...
  // Perform search
  async function executeSearch(silent = false) {
    if (!silent) {
      searchError.classList.add("hidden");
      searchEmpty.classList.add("hidden");
      searchResultsSection.classList.add("hidden");
    ...
    try {
      ...
      if (activeSearchMode === "single") {
        if (!searchFechaSingle.value) {
          throw new Error("Por favor, selecciona una fecha válida.");
        }
        data = await designacionService.buscarPorFecha(searchFechaSingle.value);
      } 
      
      else if (activeSearchMode === "range") {
        const start = searchFechaInicio.value;
        const end = searchFechaFin.value;
        if (!start || !end) {
          throw new Error("Por favor, ingresa tanto la fecha de inicio como la de fin.");
        }
        if (new Date(start) > new Date(end)) {
          throw new Error("La fecha de inicio no puede ser posterior a la fecha de fin.");
        }
        data = await designacionService.buscarPorRango(start, end);
      } 
      
      else if (activeSearchMode === "monthly") {
        data = await designacionService.buscarPorMes(parseInt(searchMonth.value), parseInt(searchYear.value));
      } 
      
      else if (activeSearchMode === "referee") {
        const refId = searchRefereeSelect.value;
        if (!refId) {
          throw new Error("Por favor, selecciona un árbitro.");
        }
        paginationNeeded = true;
        const pageData = await arbitroService.getDesignacionesByArbitro(parseInt(refId), currentPage, pageSize);
        
        const content = pageData?.content || [];
        data = content.map(item => item.Designacion || item);
        totalPages = pageData?.totalPages || 1;
        totalElements = pageData?.totalElements || data.length;
      } 
      
      else if (activeSearchMode === "court") {
        const courtId = searchCourtSelect.value;
        if (!courtId) {
          throw new Error("Por favor, selecciona una cancha.");
        }
        paginationNeeded = true;
        const pageData = await canchaService.getDesignacionesByCancha(parseInt(courtId), currentPage, pageSize);
        ...
        paginationNeeded = true;
        const pageData = await designacionService.getByEstado(parseInt(statusVal), currentPage, pageSize);
        
        const content = Array.isArray(pageData) ? pageData : (pageData?.content || []);
        data = content.map(item => item.Designacion || item);
        totalPages = pageData?.totalPages || 1;
        totalElements = pageData?.totalElements || (Array.isArray(pageData) ? pageData.length : data.length);
      }

      searchResultsList = data || [];
      
      // Update cache map for referees assignments if they came embedded
      for (const d of searchResultsList) {
      ...
      if (searchResultsList.length === 0) {
        ...
        countResults.textContent = paginationNeeded ? totalElements : searchResultsList.length;
        
        // Pagination logic display
        if (paginationNeeded && totalPages > 1) {
          paginationInfo.textContent = `Mostrando página ${currentPage + 1} de ${totalPages} (${totalElements} resultados totales)`;
      ...
      console.error(err);
      searchLoading.classList.add("hidden");
      searchErrorText.textContent = err.message || "Ocurrió un error al comunicarse con el servidor.";
  ...
  function renderResultsGrid() {
    ...
    const sorted = [...searchResultsList].sort((a, b) => new Date(a.fecha).getTime() - new Date(b.fecha).getTime());
    ...
    sorted.forEach(d => {
      ...
      const assignedRefereesHTML = `
        <div class="assigned-section hidden mt-3 p-2.5 bg-slate-50 border border-slate-100 rounded-xl">
          ...
          <div class="assigned-list-container flex flex-col gap-1">
            <div class="text-center py-2 text-slate-400 text-xs">
              <i class="ti ti-loader spin-icon text-lg block mb-1 mx-auto"></i> Cargando...
            </div>
          </div>
        </div>
      `;
      ...
      if (toggleAssignedBtn) {
        ...
        const listContainer = section.querySelector(".assigned-list-container");
        let loaded = false;
        ...
        toggleAssignedBtn.addEventListener("click", async () => {
          ...
          if (isHidden) {
            ...
            if (!loaded) {
              try {
                const list = await designadoService.getByDesignacion(id);
                state.arbitrosDesignadosMap[id] = list;
                ...
                console.error(err);
                listContainer.innerHTML = `
                  <div class="text-[11px] text-red-500 italic text-center py-2 bg-white border border-red-100 rounded-lg">
                    Error al cargar árbitros.
                  </div>
                `;
              }
            }
  ...
  async function openManageRefereesModal(id) {
    ...
    const d = searchResultsList.find(item => (item.idDesignacion || item.id) === id);
    if (!d) return;
  ...
  function renderManageAvailableReferees() {
    ...
    // Exclude referees already assigned
